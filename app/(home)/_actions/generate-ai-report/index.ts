...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  const hasPremiumPlan = true; //user.publicMetadata.subscriptionPlan === "premium";
  if (!hasPremiumPlan) {
    throw new Error("Você precisa assinar o plano premium para gerar relatórios por IA!");
  }
  ...
  const transactions = await db.transaction.findMany({
    where: {
      date: {
        gte: new Date(`${yearCurrent}-${month}-01`),
        lt: new Date(`${yearCurrent}-${month}-31`),
      },
    },
  });
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em gestão e organização de finanças pessoais e empresariais. Você ajuda pessoas e empresas a organizarem melhor as suas finanças.",
      },
      {
        role: "user",
        content,
